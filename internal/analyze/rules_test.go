...
func TestRuleThinkingBudgetRejectedFiresOn400(t *testing.T) {
	...
	ev := &store.Event{Status: 400, RespBody: []byte(`{"type":"error","error":{"type":"invalid_request_error","message":"thinking.budget_tokens is not supported"}}`)}
...
func TestRuleThinkingBudgetRejectedSilentWhenAccepted(t *testing.T) {
	budget := 1024
	meta := parse.Meta{HasThinking: true, ThinkingBudget: &budget}
	ev := &store.Event{Status: 200, RespBody: []byte(`{"type":"message"}`)}
...
func TestRuleStreamIncomplete(t *testing.T) {
	incomplete := &store.Event{CaptureComplete: false}
	...
	complete := &store.Event{CaptureComplete: true}
...
func TestRuleRateLimitedCarriesRetryAfter(t *testing.T) {
	...
	ev := &store.Event{Status: 429, RespHeaders: string(raw)}
...
func TestRuleOverloaded(t *testing.T) {
	if _, fired := ruleOverloaded(parse.Meta{}, parse.Usage{}, &store.Event{Status: 529}); !fired {
		t.Error("529 did not fire overloaded")
	}
	if _, fired := ruleOverloaded(parse.Meta{}, parse.Usage{}, &store.Event{Status: 200}); fired {
...
func TestRuleUpstreamErrorBodyFiresOnErrorObject(t *testing.T) {
	ev := &store.Event{Status: 200, RespBody: []byte(`{"type":"error","error":{"type":"overloaded_error","message":"overloaded"}}`)}
...
func TestRuleUpstreamErrorBodySilentOnCleanBody(t *testing.T) {
	ev := &store.Event{Status: 200, RespBody: []byte(`{"type":"message","content":[]}`)}
...
func TestRuleAuthKindAnomaly(t *testing.T) {
	cases := []struct {
		...
		{"admin against messages", &store.Event{AuthKind: "admin", Path: "/v1/messages"}, true},
		{"api_key on subscription account", &store.Event{AuthKind: "api_key", BillingMode: "subscription"}, true},
		{"admin against usage report", &store.Event{AuthKind: "admin", Path: "/v1/organizations/usage_report"}, false},
		{"api_key on api account", &store.Event{AuthKind: "api_key", BillingMode: "api"}, false},
		{"oauth on subscription account", &store.Event{AuthKind: "oauth", BillingMode: "subscription"}, false},
	}
	for _, c := range cases {
...
func TestRuleAPIEquivalentCostFiresOnPricedSubscriptionRow(t *testing.T) {
	usd := 0.42
	ev := &store.Event{BillingMode: "subscription", ApiEquivalentCostUSD: &usd}
...
func TestRuleAPIEquivalentCostSilentOnAPIRow(t *testing.T) {
	usd := 0.42
	ev := &store.Event{BillingMode: "api", CostUSD: &usd}
...
func TestRuleAPIEquivalentCostSilentWhenUnpriced(t *testing.T) {
	ev := &store.Event{BillingMode: "subscription", CostSource: "unpriced"}
	if _, fired := ruleAPIEquivalentCost(parse.Meta{}, parse.Usage{}, ev); fired {
