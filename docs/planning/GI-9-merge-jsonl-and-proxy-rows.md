...
1. for each `source='proxy'` row whose `request_id` is **synthetic** — the predicate is
   ...
   exists to avoid — **and** whose body yields an id: decode the body and read the id by the **same
   rule the live path applies** — `parse.ExtractUsage`'s `Usage.MessageID`, i.e. D2's precedence (the
   `message_start.message.id`, or the non-stream top-level `id` gated on the body's own `type`,
   first-seen wins), applied to the decoded body. This is a **second id-extraction site**, so it must
   use D2's one rule rather than a hand-written read: a divergence is a **silent non-merge**, and §5
   pins the two D2 cases at this level;
...
- **the deleted row's warnings** (F10.8) — `warnings.event_id` is `ON DELETE CASCADE`
  ...
  accumulates both sides'; the rekey path therefore **re-attaches the deleted row's warnings to the
  survivor** before the delete (upsert by `(event_id, kind)`), so the two paths end with the same
  warning set. **The re-attach lives in the caller**
  ([`store.go:1677-1682`](../../internal/store/store.go#L1677-L1682)), **not in `applyMergeTx`** — the
  operation is defined on the row that gets *deleted*, and `insertOrMerge`'s incoming side is a
  freshly built `Event` that was never inserted, so it structurally holds no `warnings` row to carry
  ([`store.go:1713-1717`](../../internal/store/store.go#L1713-L1717)). Reachable kinds on a JSONL row
...
4. **`ExtractMeta` stops nil-ing `PrefixHash` when a header is present** (F11.3). Today
   ...
   `SessionHeader` non-empty for the **~97%** of proxy calls that carry a header (1,364 of ~1,400, §2.6),
   while the residual **~3%** fall back to the `else` branch's hash — so **the nil-ing silently kills the
   two rules for the majority of calls**, and
...
| `cmd/clens/main_test.go` | **the file a registration breaks if it is not moved with it** (F20.1). `TestEveryCarriedOverCommandIsDispatched` asserts the dispatch map **in both directions**: the loop checks each `carriedOver` name resolves to a non-nil entry, then `len(commands) != len(carriedOver)` fails when the two sets differ in **size**. `rekey` makes that 19 against 18, so a *correct* registration turns the test red on a message about counts. Add `"rekey"` to `carriedOver` and take its comment's "the 18 subcommands" to 19. **The list is the stale side, not the assertion**: the length check is what notices a key that exists but points at nothing (a panic at dispatch rather than at build), so deleting it to go green is the wrong repair, and leaving `rekey` out of the map to keep the count is the other one |
...
| `internal/cli/cli_test.go` | add `rekey` to `TestNoCommandPrintsACredential` (~`:413`, currently **14 of the 18** dispatch names — it omits `ingest`, `refresh`, `reconcile` and `serve`) for symmetry: `rekey` prints counts, so its leak surface is nil, which is itself worth asserting. It is **not** added to `TestEveryCarriedOverCommandRunsAgainstATempStore` (~`:73`) — that table is the **11**-entry *carried-over* set by design, and a new command is the wrong thing to add to a table named for carried-over commands. Neither **of these two** tables is derived from the `commands` map and neither enumerates every subcommand, so `rekey`'s entry here is a deliberate edit rather than something a complete list would have caught. **That is a claim about this file only, and it is false of the repo**: `cmd/clens/main_test.go` keeps a complete enumeration *with* a size check, which is why it has a row of its own above (F20.1). The earlier wording generalised from this file to "no complete list exists", which is the sibling-copy error this plan has now made a fourth time — the copy that carries the contradiction was in the file nobody opened. **And its seven `newTailer` call sites gain the resolver argument** (`:600`, `:607`, `:612`, `:635`, `:646`, `:663`, `:683`) — D7 gives `newTailer` a resolver parameter, so without this the package does not compile; the only other `newTailer` callers are [`ingest.go:53`](../../internal/cli/ingest.go#L53) and [`refresh.go:95`](../../internal/cli/refresh.go#L95), both already in this table |
...
- **the two D2 cases pinned at the rekey level, not only in the `internal/parse` unit** (F18.3): a body
  with **two** `message_start` frames re-keys to the **first** id, and a body whose `type` is **not**
  `"message"` but carries a top-level `id` re-keys to **nothing** (the row stays synthetic and is
  counted in **M**). Pass 1 is a **second id-extraction site**, and neither case appears on the measured
  corpus (0 bodies with two ids, 105 of 105 non-stream bodies `type: "message"`, §2.3), so only this
  fixture can catch a pass-1 read that diverges from the live path — a divergence that is a **silent
  non-merge**.
...
`x-claude-code-session-id`** (F15.4), **plus one whose `x-clens-session` is overlong while its
`x-claude-code-session-id` is valid** (F18.4 — the one combination where D7 change 1's fill-when-absent
rule and the length rule interact: the overlong override does not supply the value, so the second header
does). Run `rekey --yes`, and assert: the first two rows' `session_id` is
now the header value; **the overlong-override row's `session_id` is its `x-claude-code-session-id`
value, not its minted `s_…`** — an implementation that tests presence with
`headers.Get("x-clens-session") != ""` satisfies the both-present and overlong-alone rows above and then
keys the session on a value the bound exists to reject (`meta.go:34-37`); **the both-present row's
`session_id` is the `x-clens-session` override, not the
...
conversation-id rows' own, the both-present row's, and the overlong-override row's) are **gone**
...
whose `source_refs` contains **both** `proxy` and `jsonl`: **0 today, expected the same ratio as check
(1)** — **~77–79%** of the post-rekey window's distinct proxy body ids, i.e. a few hundred to **~950**
rows on this DB, and the run reports the number. The target is stated in **rows** because one id
collapses to one row under D1, so the row count equals the id count; **the frozen 511 is one pass's
absolute from the smaller corpus, not the invariant** (§2.3), and an earlier pass of this plan stated
the same relation as 238 and got the unit wrong — 238 was a *pair* count over transcript **lines**,
which is why it is not the target (§2.3). (3) The row count for the rekeyed
...
- **No `replay_of` may name a row that no longer exists.** `replay_of` is a TEXT reference to an events id
  ...
  is the wrong number. **Both are zero** (measured 2026-09-21 against `~/.clens/lens.db`, 95,603 events):
  `replay_of` is `''` on every row — NULL on none — as is `replay_edits`, so **the replay feature has never
  been exercised on this database** and the collision path's re-point has **no live data behind it**. §5's
  seeded third row is therefore the *only* thing that can ever exercise the re-point: no acceptance run on
  this DB distinguishes a correct re-point from an absent one. The same query re-counts §2.6's magnitudes on
  the grown DB (2,966 `source='proxy' AND resp_body IS NOT NULL` against 724; 75,823 `jsonl:`-keyed against
  pass 3's ~71k) — growth since the evidence pass, not a contradiction, so §2's figures stand as the frozen
  reading they were taken as.
...
>   **§5's `source_refs` set assertion is what catches this one** (not the row-count assertion — both
>   shapes drop exactly one row).
...
| 2026-09-21 | Post-convergence evidence fill-in (**plan v21**, status stays **converged**). **No finding, no round, no requirement changed** — this records the one measurement §6 explicitly deferred. §6 said of the `replay_of` dangle count that "the number itself is still pending: this plan's pass had no shell and could not run the query, so it belongs in this paragraph once taken." It is taken: against the live `~/.clens/lens.db` (95,603 events, `sqlite_version` 3.50.6) the referrer count and the dangle count are **both zero** — `replay_of` is `''` on every row and NULL on none, `replay_edits` likewise. So **the replay feature has never been exercised on this database**, and the collision path's re-point has **no live data behind it**; §6 now says so, with the consequence for §5 — the seeded third row is the *only* thing that can ever exercise the re-point, because no acceptance run on this DB distinguishes a correct re-point from an absent one. The same query re-counts §2.6's magnitudes on the grown DB (2,966 `source='proxy' AND resp_body IS NOT NULL` against 724; 75,823 `jsonl:`-keyed against pass 3's ~71k) and §6 records them as **growth since the evidence pass, not a contradiction**, so §2's figures stand as the frozen reading they were taken as — the same perishable-vs-invariant split §2.3 already draws. Because no requirement, rule, or test spec moved, the round-18/19 MAJOR-free verdicts stand on the design as reviewed. **21 rows total; status: converged.** |
| 2026-09-21 | POLISH revision (finding **F20.1**; **plan v22**, status **converged**). **One MAJOR-class claim, found by Phase 4 POLISH after the loop closed** — recorded as MAJOR-class by this plan's own bar because it is a stated claim about the code that is **false against the code**, the same class as F17.1. §4's `internal/cli/cli_test.go` row asserted that "Neither table is derived from the `commands` map and **neither enumerates every subcommand**, so `rekey`'s registration is a deliberate edit in the credential table rather than a gap a complete list would have caught." **That is false, and load-bearing**: `cmd/clens/main_test.go`'s `TestEveryCarriedOverCommandIsDispatched` keeps a hand-written 18-name `carriedOver` list *and* asserts `len(commands) != len(carriedOver)`, so registering `rekey` makes it 19 against 18 and turns the test **red on a correct implementation**. The claim generalised from the one test file the plan opened to the repo as a whole — **the fourth instance of this plan's dominant defect class, a corrected claim with a surviving sibling copy, and the first where the sibling sat in a file the plan had never opened**. Three sites now move together: §4 gains a `cmd/clens/main_test.go` row naming both directions of the assertion and the repair (extend the list; the length check is what notices a key pointing at nothing, so deleting it is the wrong green), the `cli_test.go` row's claim is narrowed to a statement about **that file** with the false generalisation named, and its "10-entry" becomes **11** (the sibling test's own comment says "eleven commands"). No requirement or design rule changed — one false claim corrected and one missing file added — so the round-18/19 verdicts stand on the design as reviewed, while this row records plainly that **the loop missed a MAJOR-class claim that the next phase caught**; the bead set absorbed it in the same pass (br-GI-9-04 gains the file, the instruction, and the two wrong repairs). **22 rows total; status: converged.** |
| 2026-09-21 | Implementation cross-review, round 20 (finding **F20.5**; **plan v23**, status **converged**). The `impl-conductor` loop ran its first round over `main..HEAD` and halted on a **source-of-truth defect**: §3's re-attach bullet — and its sibling copy in `br-GI-9-04` — said the deleted row's warnings are re-attached by "**the helper**" (`applyMergeTx`). They are not: the code re-attaches in the **caller**, `rekeyOneProxyRow` ([`store.go:1677-1682`](../../internal/store/store.go#L1677-L1682)), and it **cannot** live in the helper — the operation is defined on the row that gets *deleted*, and `insertOrMerge`'s incoming side is a freshly built `Event` that was never inserted, so it structurally holds no `warnings` row to carry ([`store.go:1713-1717`](../../internal/store/store.go#L1713-L1717)). Behaviour is unaffected (the bead's own §5 fixture passes either way), so this is the story's dominant class once more — **a claim about the code that the code does not bear out** — corrected in **both** artifacts rather than one. The rekey path's obligation is unchanged; only the named site moves. **A repo-wide grep for `re-attach`/`reattach` returns no surviving sibling claim.** Round 20's four code findings (F20.1–F20.4) are untouched by this row. **23 rows total; status: converged.** |
