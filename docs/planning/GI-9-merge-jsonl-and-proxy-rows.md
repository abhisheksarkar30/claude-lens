...
3. if the new key is taken, **merge into the taker and delete the row that did not survive**, then
   ...
   a command whose sibling pass forbids the artifact must not leave it. **The collision is one of the
   run's two delete sites, so its `replay_of` re-points here too** (F15.2, §6): a row whose `replay_of`
   names the deleted proxy row's id is re-pointed to the survivor, exactly as pass 3's delete
   re-points its own, so an implementation that resolves the reverse lookup only over pass 3's `jsonl:`
   id set is caught. The taker is whatever holds the key (`getEventByRequestIDTx` filters on
...
- **a `replay_of` naming the absorbed proxy row's id re-points to the survivor** (F15.2) — the
  collision is one of the run's **two delete sites** (§6), but §5's `replay_of` case below sits at pass
  3's site only. Seed a third row whose `replay_of` names the **deleted proxy row**'s id and assert
  that after `rekey --yes` it names the **survivor** (the JSONL taker's row) — not the deleted id, and
  not cleared. An implementation that resolves the reverse lookup only over pass 3's `jsonl:` id set —
  the natural way to write it, since D4 gives the re-point a transaction only in pass 3 — stays green
...
- **Report the run's wall clock, beside the conversation's row count** (F11.4, F15.3). Pass 3
  ...
  *existence* is gated instead by §5's tailer-wiring case (`:1122-1126`), which fails today because
