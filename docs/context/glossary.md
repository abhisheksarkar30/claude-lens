...
| **`nonAnalyzeKinds`** | The four kinds emitted by a package *other* than `analyze` (`analyzer_panic`, `source_mismatch`, `cost_drift`, `quota_window_approaching`). Exists so a README check expects a kind with no rule in that package. | [internal/analyze/kinds.go](../../internal/analyze/kinds.go) |
| **`billing_mode`** | `subscription` or `api`. Comes from the **configured account**, not from the credential shape alone. | [cost-and-quota.md](cost-and-quota.md) |
