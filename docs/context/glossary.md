...
| **`capture_complete`** | Whether the capture is whole, or was narrowed by the read cap / a stream that ended early. Covers **both** bodies, request and response. Stays **true** under `--body-policy off`, where no body was captured at all: the flag reports *narrowing*, and a policy the operator set uniformly is not a narrowing. It is a *capture* flag — a capped `transcript_content` does not clear it either. | [internal/store/schema.sql](../../internal/store/schema.sql) |
...
| **`EventSummary` / `Event`** | The scalar block, and the full row that embeds it. The list routes return the summary; `*Full` / `SessionEvents` are the full-row reads. | [internal/store/types.go](../../internal/store/types.go) |
...
| **body cap** | `--body-cap-bytes` (default 262144): the most bytes stored **per body**. Bounds what `clens` keeps, never what it forwards. | [internal/config](../../internal/config/) |
